...
const Speaker = () => {
  ...
  let speakers = products.filter(product => product.category === 'speakers');
  ...
  return (
    <section>
      <div className="p-4 bg-black">
        <h1 className="text-3xl font-bold text-center mb-20 text-white">Speakers</h1>
      </div>
      ...
      {speakers.map((product, index) => (
        ...
        >
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <Link to={`/speaker/${product.id}`} className="bg-orange-400 hover:bg-orange-200 text-white font-medium rounded-sm shadow py-2 px-4 transition duration-300 ease-in-out">
              See Product
            </Link>
          </div>
        </div>
