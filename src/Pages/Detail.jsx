...
const DetailPage = () => {
  ...
  let headphones = products.filter(product => product.category === 'headphones');

  if (headphones.length >= 3) {
    const thirdHeadphone = headphones[2];
    const updatedHeadphones = [thirdHeadphone, ...headphones.slice(0, 2), ...headphones.slice(3)];
    headphones = updatedHeadphones;
  }

  return (
    <section>
      <div className="p-4 bg-black">
        <h1 className="text-3xl font-bold text-center mb-20 text-white">HeadPhones</h1>
      </div>
      ...
      {headphones.map((product, index) => (
        ...
        >
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <Link to={`/product/${product.id}`} className="bg-orange-400 hover:bg-orange-200 text-white font-medium rounded-sm shadow py-2 px-4 transition duration-300 ease-in-out">
              See Product
            </Link>
          </div>
        </div>
        ...
        {products.slice(0, 3).map((product, index) => (
          ...
          >
            ...
            <div className="content mt-16">
              ...
              <Link to={`/product/${product.id}`} className="shop-button mt-4 px-4 py-2 text-orange-500 font-bold hover:text-orange-600">Shop <span>&#x2B9E;</span></Link>
            </div>
          </div>
        ))}
      </section>
